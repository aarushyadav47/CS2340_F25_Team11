...
public class ChartFactoryTest {
    ...
    public void testCreateCategoryPieChart() {
        ...
        expenses.add(new Expense("Movie", 12.0, Category.ENTERTAINMENT, "10/22/2024", ""));
    ...
    @Test
    public void testCreateCategoryPieChartAggregatesByCategory() {
        List<Expense> expenses = new ArrayList<>();
        expenses.add(new Expense("Coffee", 5.0, Category.FOOD, "10/25/2024", ""));
        expenses.add(new Expense("Lunch", 15.0, Category.FOOD, "10/24/2024", ""));
        expenses.add(new Expense("Dinner", 20.0, Category.FOOD, "10/23/2024", ""));

        PieData pieData = ChartFactory.createCategoryPieChart(expenses);

        assertNotNull("PieData should not be null", pieData);
        assertTrue("Should aggregate expenses by category",
                pieData.getEntryCount() == 1);
    }
    ...
    @Test
    public void testCreateBudgetBarChartWithEmptyData() {
        List<Budget> emptyBudgets = new ArrayList<>();
        List<Expense> emptyExpenses = new ArrayList<>();

        BarData barData = ChartFactory.createBudgetBarChart(emptyBudgets, emptyExpenses);

        assertNotNull("BarData should not be null even with empty data", barData);
        assertTrue("BarData should have at least one entry",
                barData.getEntryCount() > 0);
    }

    @Test
    public void testCreateBudgetBarChartMatchesCategories() {
        List<Budget> budgets = new ArrayList<>();
        budgets.add(new Budget("Food Budget", 200.0, Category.FOOD, "10/01/2024", "Monthly"));
        budgets.add(new Budget("Transport Budget", 100.0, Category.TRANSPORT, "10/01/2024", "Monthly"));
        ...
        expenses.add(new Expense("Lunch", 50.0, Category.FOOD, "10/15/2024", ""));
        expenses.add(new Expense("Bus", 25.0, Category.TRANSPORT, "10/14/2024", ""));
        expenses.add(new Expense("Movie", 30.0, Category.ENTERTAINMENT, "10/13/2024", ""));

        BarData barData = ChartFactory.createBudgetBarChart(budgets, expenses);

        assertNotNull("BarData should not be null", barData);
        assertTrue("Should only match expenses to budgets with same category",
