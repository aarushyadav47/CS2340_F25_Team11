...
public class StrategyPatternTest {
    ...
    public void testSortByDateStrategy() {
        ...
        assertTrue("Expenses should be sorted by date ascending",
                expenses.get(0).getDate().compareTo(expenses.get(1).getDate()) < 0);
    }
    ...
    public void testSortByAmountStrategy() {
        ...
        assertTrue("Expenses should be sorted by amount descending",
                expenses.get(0).getAmount() > expenses.get(1).getAmount());
    }
    ...
    public void testSortByCategoryStrategy() {
        ...
        assertTrue("All expenses of same category should be grouped together",
                expenses.get(0).getCategory() == expenses.get(1).getCategory() ||
                        expenses.get(0).getCategory().getDisplayName()
                                .compareTo(expenses.get(1).getCategory().getDisplayName()) < 0);
    }
    ...
    public void testStrategyPatternInterchangeability() {
        ...
        List<Expense> expenses3 = createTestExpenses();

        ExpenseSortStrategy dateStrategy = new SortByDateStrategy();
        ExpenseSortStrategy amountStrategy = new SortByAmountStrategy();
        ExpenseSortStrategy categoryStrategy = new SortByCategoryStrategy();

        dateStrategy.sort(expenses1);
        amountStrategy.sort(expenses2);
        categoryStrategy.sort(expenses3);

        assertTrue("Different strategies should produce different orderings",
                !expenses1.equals(expenses2) || !expenses2.equals(expenses3));
    }

    @Test
    public void testSortByAmountWithEqualAmounts() {
        List<Expense> expenses = new ArrayList<>();
        expenses.add(new Expense("Expense1", 10.0, Category.FOOD, "10/25/2024", ""));
        expenses.add(new Expense("Expense2", 10.0, Category.TRANSPORT, "10/24/2024", ""));

        ExpenseSortStrategy strategy = new SortByAmountStrategy();
        strategy.sort(expenses);

        assertEquals("Both expenses should remain in list", 2, expenses.size());
    }
}
