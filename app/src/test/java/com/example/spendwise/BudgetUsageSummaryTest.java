...
public class BudgetUsageSummaryTest {
    ...
    public void testBudgetUsageSummaryCreation() {
        ...
        assertEquals("Category name should match", categoryName, summary.getCategoryName());
    ...
    public void testRemainingAmountCalculation() {
        ...
        double expectedRemaining = 150.0;
        assertEquals("Remaining amount should be allocated minus spent",
                expectedRemaining, summary.getRemainingAmount(), 0.001);
    }
    ...
    public void testUtilizationPercentage() {
        ...
        double expectedUtilization = 50.0;
        assertEquals("Utilization should be 50%",
                expectedUtilization, summary.getUtilizationPercentage(), 0.001);
    }

    @Test
    public void testUtilizationPercentageCannotExceed100() {
        BudgetUsageSummary summary = new BudgetUsageSummary(
                "budget_4", "Over Budget", "Shopping", 100.0, 150.0);

        assertEquals("Utilization should cap at 100%",
                100.0, summary.getUtilizationPercentage(), 0.001);
    }

    @Test
    public void testUtilizationPercentageWithZeroAllocation() {
        BudgetUsageSummary summary = new BudgetUsageSummary(
                "budget_5", "Zero Budget", "Other", 0.0, 50.0);

        assertEquals("Utilization should be 0 when allocation is 0",
                0.0, summary.getUtilizationPercentage(), 0.001);
    }
}
